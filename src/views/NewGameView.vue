  ...
  <div id="new-game">
    ...
    <div class="mb-2 font-bold">
      ...
      <div id="current-setup" class="mx-auto flex flex-col space-y-2">
        <div id="player-names" class="flex flex-row justify-center space-x-2 font-bold text-2xl">
          <div class="text-primary underline">{{ game.player1?.name }}</div>
          <div class="text-3xl">VS</div>
          <div class="text-secondary underline">{{ game.player2?.name }}</div>
        </div>
    ...
    <NewPlayerModal
      v-if="showModal"
      :error-msg="newPlayerError?.error"
...
const createGame = () => {
  ...
  router.replace({ path: '/game', query: { nav: 'true' } })
}
</script>
